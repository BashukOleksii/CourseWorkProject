...
namespace InventorySystem_MAUI.Service
{
    public class WarehouseService
    {
        ...
        {
            var response = await _httpClient.GetAsync($"api/warehouse{BuildQueryString(query)}");
        ...
        public async Task<byte[]> GetWarehouseReport(WarehouseQuery query)
        {
            var queryString = BuildQueryString(query);
            var response = await _httpClient.GetAsync($"api/export/warehouse-report{queryString}");
            return await response.Content.ReadAsByteArrayAsync();
        }

        private string BuildQueryString(WarehouseQuery query)
        {
